...
router.get('/oauth/callback', async (req, res) => {
  ...
  try {
    ...
    await AuthService.linkAccount(code as string, label); // This re-exchanges code? No, we already exchanged.
    ...
    const { encryptedToken, iv } = (await import('../services/SecurityService')).SecurityService.encrypt(tokenData.access_token);
...
router.get('/connect/callback', async (req, res) => {
  ...
  try {
    await AuthService.linkAccount(code as string, label as string);
    res.redirect('/dashboard');
  } catch (error) {
    console.error(error);
